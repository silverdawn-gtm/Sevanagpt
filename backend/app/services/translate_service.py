...
Three-tier pipeline: cache lookup first, then the self-hosted IndicTrans2
microservice as the primary engine, with deep-translator (Google Translate,
free, no API key) as fallback. Batches multiple texts into single API calls
via newline concatenation (~13x faster). Caches all results in the
translation_cache table using SHA-256 keys.
"""
...
async def _persist_cache_rows(rows: list[dict]) -> None:
    """Write translation-cache rows in an independent session.

    Kept separate from any caller-provided session so caching never commits
    (and thus never partially persists) the caller's in-flight transaction —
    important for the chat pipeline, which builds a conversation turn atomically.
...
async def translate_text(
    ...
    if translated and translated != source:
        await _persist_cache_rows([
            {"key": key, "source": source, "translated": translated, "tgt_lang": tgt_lang}
        ])
...
async def _cache_translations(
    texts: list[str], translated: list[str], tgt_lang: str, db: AsyncSession | None = None
) -> None:
    """Cache a batch of translations in an independent session.

    The ``db`` argument is accepted for backward compatibility but not used for
    the write — see :func:`_persist_cache_rows`.
    """
    rows = [
        {
            "key": _cache_key(src[:MAX_TEXT_LEN], tgt_lang),
            "source": src[:MAX_TEXT_LEN],
            "translated": trans,
            "tgt_lang": tgt_lang,
        }
    ...
    await _persist_cache_rows(rows)
