...
These exercise the full ORM against the DB. The Scheme models use
PostgreSQL-only column types (pgvector Vector, JSONB, ARRAY) that the
in-memory SQLite test engine cannot create, so they are marked
``integration`` and require a live Postgres/pgvector database.
"""
