...
export default function VoiceButton({ onSend, language, disabled }: VoiceButtonProps) {
  const { isListening, transcript, interimTranscript, startListening, stopListening, reset, supported } =
  ...
  // Gate rendering until mounted so server and first client render agree
  const [mounted, setMounted] = useState(false);
  useEffect(() => setMounted(true), []);

  // When recognition ends with a final transcript, send it exactly once.
  // onSend is read from a ref so a change in its identity doesn't re-trigger.
  useEffect(() => {
